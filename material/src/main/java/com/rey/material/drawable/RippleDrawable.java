...
//            Log.e("setRippleEffect", "mRippleRadius = " + String.valueOf(mRippleRadius));
            mRippleRadius = radius;
//            Log.e("setRippleEffect", "radius = " + String.valueOf(radius));
            radius = mRippleRadius / GRADIENT_RADIUS;
            mMatrix.reset();
            mMatrix.postTranslate(x, y);
            Log.e("setRippleEffect", "pre x = " + String.valueOf(x) + ", y = " + String.valueOf(y));
            Log.e("setRippleEffect", "radius = " + String.valueOf(radius));
            mMatrix.postScale(radius, radius, x, y);
            Log.e("setRippleEffect", "x = " + String.valueOf(x) + ", y = " + String.valueOf(y));
...
//        Log.v(RippleDrawable.class.getSimpleName(), "touch: " + event.getAction() + " " + mState);
            ...
            case MotionEvent.ACTION_MOVE:
                if (mState == STATE_OUT || mState == STATE_RELEASE) {
                    if (mRippleType == TYPE_WAVE || mRippleType == TYPE_TOUCH_MATCH_VIEW) {
                        Log.e("onTouch", "rippleType == wave || type_touch_match_view");
                    ...
                    Log.e("onTouch", "rippleType == type_touch");
                    if (setRippleEffect(event.getX(), event.getY(), mRippleRadius))
                        invalidateSelf();
                } else {
                    Log.e("onTouch", "type_touch_on_hold");
                }
                break;
    ...
    public static class Builder {
        ...
        private int mRippleType;
        private int mMaxRippleRadius;
        private int mRippleAnimDuration = 900;
        private int mRippleColor;
        private int mDelayClickType;
